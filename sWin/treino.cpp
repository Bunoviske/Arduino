...
void setNextSubStateVariables(uint8_t nextSubState) {
  if (nextSubState == LOGICA) {
    ...
    sb::flagSinalizaComeco = 1;
    sb::direcao = 'i';
    sb::atual_pos = 0; //tres variaveis devem ser zeradas ("reset" do arduino)
...
uint8_t pause() {
  ...
  if (sb::goPause == 0) { //so entra quando for despausado
    ...
    if (qtdPiscinas == 0) { //chamar estado que sinaliza comeco
      qtdPiscinas++;
      digitalWrite(PIN_ESP, LOW);
      if (sb::flagSinalizaComeco == 1 ) {
        setNextSubStateVariables(SINALIZA_COMECO);
        return TREINO;
      }
      //se entrou aqui é pq nao houve intervalo entre series!
      sb::flagSinalizaComeco = 1; //a principio considera-se que a prox serie nao tem descanso_serie = 0. Caso tenha, logicaIntervalo ira modificar sinalizaComeco para 0!!!
...
uint8_t logica() { //inicializa a logica do timer. so deve ser chamada quando percorrer ate o ultimo pixel do trecho
  ...
  if (qtdPiscinas == 0) { //se for a primeira vez que entrou no estado de logica, inicializa sinalizaComeco e o prox estado eh pause automaticamente (ver prox linhas)
    if (sb::flagSinalizaComeco == 1 ) setSinalizaComeco();
...
void logicaIntervalo() { //nao eh um sub estado mas esta muito atrelado ao estado INTERVALO
  ...
  if (qtdPiscinas == sb::serieAtual.repeticao * (sb::serieAtual.distancia / (sb::NUMPIXELS / 10))) { //se a qtd de piscinas na serie atingir o maximo, termina a serie
    ...
    if (auxTime < 0) { //caso em que nao deve haver tempo de espera entre as series e nem sinalizar o comeco
      OCR1B = 0;
      qtdMaxInterrupts = 1;
      sb::flagSinalizaComeco = 0; //a primeira repeticao da proxima serie que o arduino receber estara indicada para nao chamar sinalizaComeco!!!
    }
...
uint8_t treinar() {
  ...
  if (FLAG_ISR == 'A') { //comp A
    ...
    if (sb::atual_pos + offset <= sb::NUMPIXELS - 1 && sb::atual_pos + offset >= 0)   fita[sb::atual_pos + offset] = fita[sb::atual_pos]; //acende de 4 em 4 pixels
    if (sb::atual_pos + 2*offset <= sb::NUMPIXELS - 1 && sb::atual_pos + 2*offset >= 0)   fita[sb::atual_pos + 2*offset] = fita[sb::atual_pos]; 
    if (sb::atual_pos + 3*offset <= sb::NUMPIXELS - 1 && sb::atual_pos + 3*offset >= 0)   fita[sb::atual_pos + 3*offset] = fita[sb::atual_pos]; 
    if (sb::atual_pos + 4*offset <= sb::NUMPIXELS - 1 && sb::atual_pos + 4*offset >= 0)   fita[sb::atual_pos + 4*offset] = fita[sb::atual_pos]; 
    
    if (sb::atual_pos - offset <= sb::NUMPIXELS - 1 && sb::atual_pos - offset >= 0)   fita[sb::atual_pos - offset] = CRGB::Black; //apaga pixel anterior
    sb::atual_pos += offset;

    if (sb::atual_pos == final_pos) {
...
void setLedColor(float tam_atual, float tempo_atual, float tam_ante, float tempo_ante) {
  ...
  if (vel_atual > vel_ante)  { //caso rapido (vel atual maior que anterior)
    ...
    fita[sb::atual_pos] = CRGB::Red; //mesma velocidade
  }
...
void setFirstLed() { //na primeira serie e apos os delays, eh necessario chamar essa funcao, pois o primeiro led nao precisa esperar para ser aceso.
  ...
  fita[sb::atual_pos] = CRGB::Red; //o primeiro pixel (posicao 0 ou NUMPIXELS - 1) smp deve receber verde
  FastLED.show();
  TCNT1 = 0;
  tempo = millis();
  setComp();
...
void setFinalPos(int16_t pixelsRange) {
  ...
  final_pos = sb::atual_pos + pixelsRange - offset; //offset eh positivo na ida e negativo na volta
}
