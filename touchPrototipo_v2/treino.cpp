...
uint8_t ledConfig() {
  ...
  clearAllLeds();
  FastLED.addLeds<WS2811, PIN_WS2811, BRG>(gb::fita, MAX_LEDS); // tamanho da piscina definido pelo app
...
uint8_t logica() { //inicializa a logica do timer. so deve ser chamada quando percorrer ate o ultimo pixel do trecho
  ...
  if (gb::direcao == 'i') { //ida
    ...
    float auxTime = ((float)gb::serieAtual.TimeIda) / (pixelsTrecho - 1); //menos 1 pois sao numPixels - 1 intervalos de tempo
    ...
    float auxTime = ((float)gb::serieAtual.TimeVolta) / (pixelsTrecho - 1); //menos 1 pois sao numPixels - 1 intervalos de tempo
...
uint8_t treinar() {
  ...
  if (FLAG_ISR == 'A') { //comp A
    ...
    if (gb::atual_pos + offset <= MAX_LEDS - 1 && gb::atual_pos + offset >= 0)   gb::fita[gb::atual_pos + offset] = gb::fita[gb::atual_pos]; //acende um led por vez
    gb::fita[gb::atual_pos] = CRGB::Black; //apaga pixel anterior
    FastLED.show();

    //if (gb::atual_pos - offset <= MAX_LEDS - 1 && gb::atual_pos - offset >= 0)   gb::fita[gb::atual_pos - offset] = CRGB::Black; //apaga pixel anterior
    gb::atual_pos += offset;
    ...
    if (gb::atual_pos == final_pos) {
      ...
      if (gb::direcao == 'i') gb::direcao = 'v';
        ...
        gb::direcao = 'i';
        if (qtdIdas != 2 * gb::serieAtual.repeticoes) updateRepeticao((uint8_t)((qtdIdas) / 2) + 1); //quando a ultima ida for completada, nao deve-se chamar updateRepeticao!
      }
    ...
    if (offset == -2) {
      ...
      if (gb::direcao == 'i') {
        ...
        for (; offset > aux; offset--) {
          gb::fita[offset] = CRGB::Black;
        }
      }
      else {
        for (; offset > aux; offset--) {
          gb::fita[MAX_LEDS - offset - 1] = CRGB::Black;
        }
      }
      FastLED.show();
...
void setFirstLed() { //na primeira serie e apos os delays, eh necessario chamar essa funcao, pois o primeiro led nao precisa esperar para ser aceso.
  ...
  gb::fita[gb::atual_pos] = CRGB::Red; //o primeiro pixel (posicao 0 ou NUMPIXELS - 1) smp deve receber verde
  FastLED.show();
  TCNT1 = 0;
  setComp();
...
void clearAllLeds() {
  FastLED.addLeds<WS2811, PIN_WS2811, BRG>(gb::fita, MAX_LEDS);
  FastLED.clear();
  FastLED.show();
  delay(100);
}
